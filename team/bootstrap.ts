...
export function authorFleetPresets(home: string): Record<string, { presetYml: string; cordisYml: string }> {
  ...
  for (const profile of ROSTER) {
    ...
    const cordisContent = [
      ...
      '# Base tool catalog mirrors the shipped standard preset — the host provides',
      '# infrastructure (fs sandbox, web search provider, etc.) but model-facing tool',
      '# wrappers must be registered by the preset into the agent scope.',
      '',
      '# ── identity ────────────────────────────────────────────────────────────────',
      '',
      '- id: persona',
      ...
      '# ── compaction ──────────────────────────────────────────────────────────────',
      '',
      '# Compaction: auto-compact at 40% context window pressure, retain 20% verbatim.',
      '# Summarization uses the same provider/model as the conversation.',
      '- id: compaction',
      '  name: cordis:group',
      '  group: true',
      '  isolate:',
